...
sumLast2 :: Int -> Int
sumLast2 n = uncurry (+) digitTuples
  where
    lastTwo = mod n 100
    digitTuples = divMod lastTwo 10
...
firstDigit n 
  | n >= 0     = go n True
  | otherwise  = go (abs n) False
  where
    go :: Int -> Bool -> Int 
    go n pos
      | pos && (div n 10 == 0)     = n
      | not pos && (div n 10 == 0) = negate n
      | otherwise                  = go (div n 10) pos
